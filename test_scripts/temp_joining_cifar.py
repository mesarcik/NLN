...
args = Namespace(input_shape=(8,8,3),
                  rotate=False,
                  patches=True,
                  patch_x = 8,
                  patch_y=8,
...
def plot_recon(test_images, x_out, mean_neighbours,  x_hat,args,ind=0):
    # plot 3 plots, input, patches, patches out 
    ...
    plt.close(fig)
    AXS[0].imshow(recon_in[ind,...], vmin=0, vmax= 1)
    AXS[0].set_title('Joined patches')
    ...
    plt.close(fig)
    AXS[1].imshow(recon_out[ind,...], vmin=0, vmax= 1)
    AXS[1].set_title('Output patches')

    fig, axs = plt.subplots(1,1)
    recon_out = reconstruct(mean_neighbours,test_images,args)
    axs.imshow(recon_out[ind,...].astype('int'),vmin=0, vmax=1)
    fig.suptitle('NLN Images')
    fig.savefig('/tmp/4_NLN')
    plt.close(fig)
    AXS[2].imshow(recon_out[ind,...], vmin=0, vmax= 1)
    AXS[2].set_title('NLN Images')

    fig, axs = plt.subplots(1,1)
    er = recon_in[ind,...] - recon_out[ind,...] 
    axs.imshow(er.astype('int'),vmin=0, vmax=1)
    fig.suptitle('Error patches')
    fig.savefig('/tmp/5_error')
    plt.close(fig)
    AXS[3].imshow(er, vmin=0, vmax= 1)
    AXS[3].set_title('Error patches')
    ...
    plt.close(fig)
    AXS[4].imshow(er, vmin=0, vmax= 1)
    AXS[4].set_title('Error patches threshodled >0.05')
...
def main():
    # Load data 
    _,train_images, train_labels, test_images, test_labels = load_cifar10(args)
    ...
    n_patches = 32//args.input_shape[0]

    x_hat = ae(test_images[0:5000,...].astype('float32')).numpy()

    x_hat_train = ae(train_images[0:5000,...].astype('float32')).numpy()

    z_query,error_query = get_error('AE', ae, test_images[0:5000], return_z = True)
    z,_ = get_error('AE', ae, train_images[0:5000,...], return_z = True)

    nbrs = neighbors.NearestNeighbors(#radius=args.radius[0], 
                                      n_neighbors= args.neighbours[0],
                                      algorithm='ball_tree',
...
#    neigh_img 
    ...
    plot_recon(np.zeros([5000,32,32,3]), x_out, mean_neighbours, x_hat,args,ind=0)
