...
@tf.function
def train_step(ae,discriminator, x,y):
...
def train(ae,discriminator, train_dataset,train_dataset_labels,test_images,test_labels,args):
    ae_loss,d_loss, g_loss,aucs = [], [], [],[]
    for epoch in range(args.epochs):
        start = time.time()
        for image_batch,labels_batch in zip(train_dataset,train_dataset_labels):
        ...
        save_checkpoint(ae,epoch,args,'DAE_labels','ae')
        save_checkpoint(discriminator, epoch, args,'DAE_labels','disc')

        ae_loss.append(auto_loss)
        d_loss.append(disc_loss)
        g_loss.append(gen_loss)

        roc_auc,f1 = get_classifcation('DAE_labels',
                                       [ae,discriminator],
                                       test_images,
                                       test_labels,
                                       args.anomaly_class,
    ...
    generate_and_save_images(ae,epoch,image_batch[:25,...],labels_batch[:25,...], 'DAE_labels',args)
...
def main(train_dataset,train_dataset_labels,train_images,train_labels,test_images,test_labels,args):
    ...
    auc_recon ,f1_recon = get_classifcation('DAE_labels',
                                             [ae,discriminator],
                                             test_images,
                                             test_labels,
                                             args.anomaly_class,
